...
class CheckinsHelperTest < ActionView::TestCase #< ActiveSupport::TestCase
	...
	test "#buildCheckinLabel" do
		...
		realPatchLink = buildCheckinLabel(@habit1, "red", (Date.new(2016, 9, 26).strftime("%Y-%m-%d")))
		...
		realPostLink = buildCheckinLabel(@habit1, "red", (Date.new(2016, 9, 27).strftime("%Y-%m-%d")))
...
private
	def create_old_checkins(status)
		...
		@checkin1 = Checkin.create(habit: @habit1, status: status, date: Date.new(2016, 9, 26).strftime("%Y-%m-%d"))
	end
